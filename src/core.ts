...
function isCompleteMint(mintId: string): boolean {
  ...
  return (mintEvent && mintEvent.sender) !== null // sufficient checks
}
...
export function handleTransfer(event: Transfer): void {
  ...
  if (event.params.from.toHex() == ADDRESS_ZERO && pair) {
    // update total supply
  ...
  if (pair && event.params.to.toHex() == pair.id) {
...
export function handleSync(event: Sync): void {
  ...
  if (pair) {
    ...
    if (pancake && pair && token0 && token1) {
      pancake.totalLiquidityBNB = pancake.totalLiquidityBNB.minus(
      ...
      if (bundle) {
        ...
        pancake.totalLiquidityBNB = pancake.totalLiquidityBNB.plus(
          trackedLiquidityBNB,
        )
        pancake.totalLiquidityUSD = pancake.totalLiquidityBNB.times(
          bundle.bnbPrice,
        )
...
export function handleBurn(event: Burn): void {
  ...
  if (pair && pancake) {
    ...
    if (token0 && token1) {
      ...
      // get new amounts of USD and BNB for tracking
      let bundle = Bundle.load('1')
      if (token0.derivedBNB && token1.derivedBNB && bundle && burn) {
        let amountTotalUSD = token1.derivedBNB
          .times(token1Amount)
          .plus(token0.derivedBNB.times(token0Amount))
          .times(bundle.bnbPrice)
...
export function handleSwap(event: Swap): void {
  ...
  if (pair) {
    ...
    if (token0 && token1) {
      ...
      if (token0.derivedBNB && token1.derivedBNB && bundle) {
        ...
        if (pancake) {
          ...
          pancake.untrackedVolumeUSD = pancake.untrackedVolumeUSD.plus(
            derivedAmountUSD,
          )
          pancake.totalTransactions = pancake.totalTransactions.plus(ONE_BI)
          ...
          pancakeDayData.dailyVolumeUSD = pancakeDayData.dailyVolumeUSD.plus(
            trackedAmountUSD,
          )
          pancakeDayData.dailyVolumeBNB = pancakeDayData.dailyVolumeBNB.plus(
            trackedAmountBNB,
          )
          pancakeDayData.dailyVolumeUntracked = pancakeDayData.dailyVolumeUntracked.plus(
            derivedAmountUSD,
          )
          pancakeDayData.save()
          ...
          token0DayData.dailyVolumeBNB = token0DayData.dailyVolumeBNB.plus(
            amount0Total.times(token0.derivedBNB as BigDecimal),
          )
          ...
          token1DayData.dailyVolumeBNB = token1DayData.dailyVolumeBNB.plus(
            amount1Total.times(token1.derivedBNB as BigDecimal),
          )
          token1DayData.dailyVolumeUSD = token1DayData.dailyVolumeUSD.plus(
            amount1Total
              .times(token1.derivedBNB as BigDecimal)
              .times(bundle.bnbPrice),
          )
          token1DayData.save()
        }
