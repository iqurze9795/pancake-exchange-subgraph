  ...
  for (let i = 0; i < WHITELIST.length; ++i) {
    ...
    if (pairAddress.toHex() != ADDRESS_ZERO) {
      ...
      if (
        ...
        const derivedBNB = token1 ? token1.derivedBNB : null
        return pair.token1Price.times(derivedBNB as BigDecimal) // return token1 per our token * BNB per token 1
      }
      if (
        pair &&
        ...
        const derivedBNB = token0 ? token0.derivedBNB : null
        return pair.token0Price.times(derivedBNB as BigDecimal) // return token0 per our token * BNB per token 0
      }
    }
  }
...
export function getTrackedVolumeUSD(
  ...
  if (token0 && token1 && token0.derivedBNB && token1.derivedBNB) {
    let price0 = token0.derivedBNB.times(bundle.bnbPrice as BigDecimal)
    let price1 = token1.derivedBNB.times(bundle.bnbPrice as BigDecimal)
